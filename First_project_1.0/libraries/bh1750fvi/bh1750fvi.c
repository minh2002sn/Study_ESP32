...
esp_err_t BH1750_Init(BH1750_DATA_t *bh1750, uint32_t i2c_sda_pin, uint32_t i2c_scl_pin, uint8_t i2c_num, uint8_t addr_pin_state)
{
    ...
    esp_err_t ret = i2c_param_config(bh1750->i2c_num, &conf);
    if(ret != ESP_OK) return ret;
    ret = i2c_driver_install(bh1750->i2c_num, conf.mode, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0);
    return ret;
}
...
esp_err_t BH1750_OneShotRead(BH1750_DATA_t *bh1750)
{
    ...
    esp_err_t ret = i2c_master_write_to_device(bh1750->i2c_num, bh1750->address, tx_data, 1, I2C_MASTER_TIMEOUT_MS/portTICK_PERIOD_MS);
    if(ret != ESP_OK) return ret;

    vTaskDelay(120/portTICK_PERIOD_MS);

    uint8_t rx_data[2] = {};
    ret = i2c_master_read_from_device(bh1750->i2c_num, bh1750->address, rx_data, 2, I2C_MASTER_TIMEOUT_MS/portTICK_PERIOD_MS);
    if(ret != ESP_OK) return ret;
    bh1750->illu_value = ((uint16_t)rx_data[0] << 8) | rx_data[1];
